...
<!--   <link rel="stylesheet" href="${ctx}/css/roomset/roomset.css" type="text/css"></link> -->
  ...
  <div class="container" >
    ...
    <div class="span5">
	    <div class="row-fluid">
		    <label class="labelroomnumber">用户邮箱：</label>
       ...
       <table class="table table-condensed table-bordered table-striped" id="tableid">
	      ...
	      <tbody id="tbody">
	        <c:forEach items="${list.result}" var="item">
		        <tr>
		          <td><input type="checkbox" name="mail" value="${item.mail}"></td>
		          <td>${item.userName}</td>
		          <td>${item.passWord}</td>
		          <td>${item.userType}</td>
 ...
 $(document).ready(function () {
   ...
   var chk_value=[];
  	...
  	if(chk_value!=""){
		if(chk_value.toString().indexOf(",")>0){
		   ...
		   parent.document.getElementById("Mainid").src='${ctx}/Passenger/toupdate.do?mail='+chk_value;
		}
	}else{
	  alert("请选择一条数据进行修改");
	}
   ...
   var chk_value=[];
     ...
     if(flag){
  	  parent.document.getElementById("Mainid").src='${ctx}/Passenger/delete.do?mail='+chk_value;
  	}
  	}else{
