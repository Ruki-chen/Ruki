...
.comment{
 ...
 <a id="textkaka" href="#lvke" style="display:none" data-toggle="modal" class="btn btn-info btn-small" onclick="getMusicList()"><li class="icon-plus icon-white"></li>播放列表</a>
    <div id="main">
    <div id="left"> 
    <img class="tt" src="<%=request.getContextPath()%>${singerimfo.photoPath}" style="height: 500px;width:700px;" />  <br>
         <input id="photoPath_id"  type="text" style="display:none;" value="${singerimfo.photoPath}" >  
         <input id="songName" type="text"  style="display:none"   value="${songimfoPo.songName}"> 
         <input id="songPath" type="text"  style="display:none"   value="${songimfoPo.songPath}"> 
 ...
 <c:forEach items="${resultList}" var="item" varStatus="status">
    <tr>
      ...
      <c:if test="${item.songCommentStuts == 'false'}"><img  onclick="addGoodNumber(${status.index+1})" class="wrap" src="<%=request.getContextPath()%>/img/点赞.PNG" /></c:if>   
      <c:if test="${item.songCommentStuts == 'true'}"><img  onclick="reduceGoodNumber(${status.index+1})" class="wrap" src="<%=request.getContextPath()%>/img/取消点赞.jpg" /></c:if>   
     ...
     <td style="display:none"><input id="commentId_${status.index+1}" type="text"  style="display:none"   value="${item.id}"> </td>
       <td style="display:none"><input id="songCommentStutsId_${status.index+1}" type="text"  style="display:none"   value="${item.songCommentStuts}"> </td>
 ...
 $(document).ready(function () {
  ...
  function deleteComment(t) {
		...
		if (t==null) {
			...
			//  id = ( right2 == "" )? $("#restore_commentId_"+t).text() :$("#commentId_"+t).val();
			//  flag = ( right2 == "" )?  "replay" :"publish";;
		}
		 var songPath = $("#songPath").val();
		 $.ajax({                                                      
	         cache:false,                                            
	         type: "POST",                                          
	         url: '${ctx}/SongComment/deleteComment.do',      
	         data:"id="+id+"&parentCommentId="+parentCommentId,                      
	         async:false,                                         
	         success: function (result) {
	        	 if(result) {
	        		 parent.document.getElementById('Mainid').src='${ctx}/SongComment/showComment.do?songPath='+ songPath+'&flag='+flag;     
	       	     }
	         },
	         error: function(data) {      }
	     }); 
  ...
  function addGoodNumber(t) {
	...
	if (t==null) {
	 ...
	 $.ajax({                                                      
         cache:false,                                            
         type: "POST",                                          
         url: '${ctx}/SongComment/addGoodNumber.do',      
         data:"id="+id,                      
         async:false,                                         
         success: function (result) {
        	 if(result) {
 ...
 function reduceGoodNumber(t) {
	...
	if (t==null) {
	 ...
	 $.ajax({                                                      
         cache:false,                                            
         type: "POST",                                          
         url: '${ctx}/SongComment/reduceGoodNumber.do',      
         data:"id="+id,                      
         async:false,                                         
         success: function (result) {
        	 if(result) {
  ...
  function publishComments() {
	...
	//	 $("#Tips").val("不能发布空消息");
	 ...
	 $.ajax({                                                      
         cache:false,                                            
         type: "POST",                                          
         url: '${ctx}/SongComment/publishComments.do',      
         data:"userComment="+userComment+"&songName="+songName+"&parentCommentId="+parentCommentId,                      
         async:false,                                         
         success: function (result) {                         
       	     if(result) {
       	    //	 $("#Tips").val("发布成功");
  ...
  function restoreComment(t) {  
	  ...
	  var songCommentStuts = $("#songCommentStutsId_"+t).val();
