...
.center{
  ...
  <div class="container" >
    ...
    <div class="span5">
	    <div class="row-fluid">
		    <label class="labelroomnumber">歌曲名字：</label>
		    <form action="${ctx}/Song/tolist.do" method="post" style="float: left;">
			   <input id="txtnameid" name="txtname" class="textone roomnumberwidth" style="border-radius:0px; border-top-left-radius:4px; border-bottom-left-radius:4px;height:26px;" type="text" placeholder="请输入歌曲名字" value="${txtname}">
       ...
       <table class="table table-condensed table-bordered table-striped" id="tableid"  style="table-layout: fixed;" >
	      <thead class="theadone">
	        <tr>
	          ...
	          <th class="wrap" style="text-align:center;vertical-align:middle;">歌词</th>
	          <th class="wrap" style="text-align:center;vertical-align:middle;">专辑名字</th>
             ...
             <th class="wrap" style="text-align:center;vertical-align:middle;">歌曲</th>
	        </tr>
	      </thead>
	      <tbody id="tbody">
	        <c:forEach items="${list.result}" var="item">
		        <tr>
		          ...
		          <td class="wrap" style="text-align:center;vertical-align:middle;">${item.lyric}</td>
		          <td class="wrap" style="text-align:center;vertical-align:middle;">${item.albumName}</td>
		          ...
		          <audio controls>
                     <source src="<%=request.getContextPath()%>${item.songPath}">
                         您的浏览器不支持 audio 元素。
                      </audio></td>
		        </tr>
	        </c:forEach>
 ...
 $(document).ready(function () {
   ...
   function addfunction(){
     parent.document.getElementById('Mainid').src='${ctx}/Song/toadd.do';
   }
   ...
   var chk_value=[];
  	...
  	if(chk_value!=""){
		if(chk_value.toString().indexOf(",")>0){
		   ...
		   parent.document.getElementById("Mainid").src='${ctx}/Song/toupdate.do?id='+chk_value;
		}
	}else{
	  alert("请选择一条数据进行修改");
	}
   ...
   var chk_value=[];
     ...
     if(flag){
  	  parent.document.getElementById("Mainid").src='${ctx}/Song/delete.do?id='+chk_value;
  	}
  	}else{
  ...
  $(".tcdPageCode").createPage({
     ...
     location.href="${ctx}/Song/tolist.do?currentPage="+p+"&txtname="+txtname;
     }
   });
